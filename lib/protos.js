...
exports.load = function(name, locals, fn){
  ...
  switch (template.type) {
    case 'img':
      attrs.width = 1;
      attrs.height = 1;
      this.debug('load image %o', attrs);
      el = loadImage(attrs, fn);
      break;
    case 'script':
      var src = attrs.src;
      this.debug('load script %o', attrs);

      // IE8-6 emit `complete|loaded` before the script
      // is evaluated, so we have to make sure it is loaded
      // by calling the integration `.loaded()`.
      el = loadScript(attrs, function(err){
        ...
        // retry
        function retry(){
          if (!self.loaded()) return tid = setTimeout(retry, 100);
      ...
      break;
    case 'iframe':
      this.debug('load iframe %o', attrs);
